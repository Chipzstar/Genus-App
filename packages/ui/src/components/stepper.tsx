		...
		>
			<div
				...
				className={cn(
					...
					"flex items-center",
					variant === "line" && "border-s-[3px] py-2 ps-3 data-[active=true]:border-blue-500",
	...
	return (
		<div
			...
			className={cn(
				...
				"[&:not(:last-child)]:after:transition-all [&:not(:last-child)]:after:duration-200",
				"[&:not(:last-child)]:after:h-[5px] [&:not(:last-child)]:after:bg-border [&:not(:last-child)]:after:content-['']",
				variant === "circle-alt" &&
				...
				variant === "circle" && "[&:not(:last-child)]:after:flex-1",
				variant === "line" && "flex-1 flex-col border-t-[3px] data-[active=true]:border-blue-500",
				isCompletedStep &&
					"[&:not(:last-child)]:after:bg-blue-500 [&:not(:last-child)]:after:data-[invalid=true]:bg-destructive",
...
const StepButtonContainer = ({
	...
	const { clickable, isLoading: isLoadingContext, variant, styles, size } = useStepper();
...
const StepIcon = React.forwardRef<HTMLDivElement, StepIconProps>((props, ref) => {
	...
	return React.useMemo(() => {
		...
		return (
			<span ref={ref} key="label" className={cn("text-md text-center font-bold")}>
				{(index ?? 0) + 1}
			</span>
		);
